						...
						<tbody>														
							@foreach ($module->fields as $field)
								<tr>
									...
									<td>
										<form id="listing_view_cal" action="{{ url('module_field_listing_show') }}">
									...
									<td style="min-width:60px;">
										<a href="{{ url('module_fields/'.$field['id'].'/edit') }}" class="btn btn-edit-field btn-warning btn-xs" style="display:inline;padding:2px 5px 3px 5px;" id="edit_{{ $field['colname'] }}"><i class="fa fa-edit"></i></a>
										<a href="{{ url('module_fields/'.$field['id'].'/delete') }}" class="btn btn-edit-field btn-danger btn-xs" style="display:inline;padding:2px 5px 3px 5px;" id="delete_{{ $field['colname'] }}"><i class="fa fa-trash"></i></a>
										@if($field['colname'] != $module->view_col)
											<a href="{{ url('modules/'.$module->id.'/set_view_col/'.$field['colname']) }}" class="btn btn-edit-field btn-success btn-xs" style="display:inline;padding:2px 5px 3px 5px;" id="view_col_{{ $field['colname'] }}"><i class="fa fa-eye"></i></a>
										@endif
									</td>
								</tr>
							@endforeach
...
$(function () {
	...
	$("#sortable_module_fields").sortable({
		update: function(event, ui) {
			...
			$.ajax({
				url: "{{ url('save_module_field_sort') }}/"+{{ $module->id }},
	...
	$("#generate_update").on("click", function() {
		...
		$fa.addClass("fa-spin");
		$.ajax({
			url: "{{ url('module_generate_update') }}/"+{{ $module->id }},
			method: 'GET',
			success: function( data ) {
				$fa.removeClass("fa-refresh");
				$fa.removeClass("fa-spin");
	...
	$("#generate_migr_crud").on("click", function() {
		...
		$fa.addClass("fa-spin");
		$.ajax({
			url: "{{ url('module_generate_migr_crud') }}/"+{{ $module->id }},
			method: 'GET',
			success: function( data ) {
				$fa.removeClass("fa-refresh");
				$fa.removeClass("fa-spin");
	...
	$('.Switch.Ajax').click(function() {
		...
		$.ajax({
			type: "POST",
			url : "{{ url('module_field_listing_show') }}",
			data : {
				_token: '{{ csrf_token() }}',
